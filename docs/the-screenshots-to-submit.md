...
4. The exact consent/digest and the distinct claim, scheduled and confirmed collection states.
5. Evidence bundle and recovery: source references, run/revision, provider/mode, observed outcome,
   record history and explicit limits.
...
Configured Opus 5 is not a model-call receipt and an optional critic is not proof of an active
critic Lambda. No real identities, credentials, account details or customer assets belong in shot.
