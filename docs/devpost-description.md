...
why they are disabled; approval, for example, reads "Available once you tick the box above."
Frontend CI runs the browser journeys at desktop and mobile viewport sizes.
...
The **Strands Agents SDK** runs the agent loop of each specialist whose deterministic rules have not
already refused, and dispatches its tool calls. Merismos registers its own guard on the SDK's
`BeforeToolCallEvent` hook. The guard cancels any tool call the deployed role may not make, before
the tool runs. In CI, removing the guard lets a negative-control publish call reach the tool.
Replacing Strands with a module that fails when used makes the offline demo journey and the
...
- Amazon CloudFront serves the site from a private, versioned Amazon S3 bucket and passes API
  requests, uncached, to the API.
...
- Three AWS IAM roles separate the reader, evaluator and writer functions; the runner runs under the
  reader role. EventBridge Scheduler has its own role that can only invoke the runner and write to
  its dead-letter queue. An AWS Secrets Manager value is a boundary canary that the publish path
  never reads.
...
An architecture overview is in `README.md`, and the detailed flow and infrastructure diagrams are in
`docs/architecture.md` and `docs/infrastructure.md`, in the submitted repository.
...
claimed. The only latency figure is a sandbox HTTP sample taken on 2026-09-13, when the deployed
frontend and backend were both at commit `cb97c9e`: one workstation, 10 samples 15 s apart, 0
failures, and a median of 426 ms for a scripted sandbox run (max 2,628 ms). It is not a load test,
browser render time or Lambda cold-start time. Live mode was not measured. The dated
`docs/live-run-2026-09-02.md` in the submitted repository records one earlier live specialist read
at its own checkpoint. The contradictory historical offer-4471 record remains disclosed in
`README.md` in the submitted repository; it is not silently repaired.
...
HTTP API at desktop and mobile viewport sizes in Chromium, and three of those spec files (cold
start, the sandbox decision journeys, and session return or expiry; seven tests) in Playwright
WebKit with an iPhone 13 preset, which is not real-device Safari. A push to main publishes the
frontend and reruns the desktop and mobile Chromium journeys against the deployed site in the
scripted sandbox. The backend deploys only by manual dispatch. Human acceptance testing, real-device
Safari testing and a timed first-use test have not been done. New testbook cases and human signoff
stay NOT_RUN until actually tested.
...
The code was implemented for Merismos during the submission period, as the disclosure in `README.md`
...
pipeline describe that tool, not the application's runtime. `README.md` and `docs/` in the submitted
repository hold the exact validation, retained evidence IDs and deployment limitations. This file is
