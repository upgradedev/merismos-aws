...
export function DispatchWorkspace({ data, selected, filter, unit, today, busy, mutate, pickup, onPickupChange }: { data: Workspace; selected: string; filter: Filter; unit: string; today: string; busy: boolean; mutate: Mutate } & PickupSelection) {
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">DECIDE THE SPLIT</p><h1>{row?.offer.title || 'Decide the split'}</h1><p>{row ? `${row.offer.id} · ${row.offer.donor}` : 'Select an offer to review its evidence and next decision.'}</p></div><a className="button secondary" href="#/offers/new">+ Add offer</a></div>
    <div className="workspace-context"><a href="#/offers" className="back-link">← All offers</a><span>{source.offers.length} offers · {row ? '1 offer in focus' : 'No offer in focus'}</span><a href={routeLink('/records', { offer: selected, pickup })}>Inspect records →</a>{row && <button className="secondary" onClick={() => document.getElementById('next-decision')?.focus()}>Go to next decision ↓</button>}</div>
    {source.conflicts > 0 && <p className="notice">Conflicting source identities were withheld. Refresh and review before acting.</p>}
    <div className="dispatch-grid"><aside id="next-decision" tabIndex={-1} className="decision-pane" aria-label="Decision and dispatch"><div className="pane-heading"><h2>Your decision</h2></div>{row ? <><p className="decision-context">{row.offer.id} · {row.offer.title}</p>{hiddenSelection ? <p className="notice">Show this offer's evidence before acting. <a href={routeLink('/workspace', { offer: selected, pickup })}>Review selected offer</a>.</p> : <><DecisionPanel key={`${data.mode}-${selected}-${row.result.run_id}-${row.plan?.digest}-${row.plan?.evidence_digest}-${data.version}`} row={row} data={data} busy={busy} mutate={mutate}/>
    ...
    <div className="dispatch-main">{row && <DispatchJourney row={row} data={data}/>}<section className="panel intake-stream" aria-label="Intake and allocation"><div className="pane-heading"><h2>Offers</h2><span className="badge">{filters[filter]}{unit ? ` · ${unit}` : ''}</span></div><div className="stream-search"><label>Search offers<input type="search" placeholder="Offer, donor or identifier" value={query} onChange={e => setQuery(e.target.value)}/></label></div>
