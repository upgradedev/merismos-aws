...
export function DispatchWorkspace({ data, selected, filter, unit, today, busy, mutate, pickup, onPickupChange }: { data: Workspace; selected: string; filter: Filter; unit: string; today: string; busy: boolean; mutate: Mutate } & PickupSelection) {
  ...
  return <>
    ...
    <section className="panel intake-stream" aria-label="Intake and allocation"><div className="pane-heading"><h2>Offers</h2><span className="badge">{filters[filter]}{unit ? ` · ${unit}` : ''}</span></div><div className="stream-search"><label>Search offers<input type="search" placeholder="Offer, donor or identifier" value={query} onChange={e => setQuery(e.target.value)}/></label></div>
