...
export function App() {
  ...
  return <div className="app-shell"><a href="#main" className="skip-link" onClick={e => { e.preventDefault(); document.getElementById('main')?.focus(); }}>Skip to main content</a>
    ...
    <div className="workspace"><header className="topbar"><span>Community coordination</span><div className="mode-control"><label htmlFor="mode">Workspace</label><select id="mode" value={mode} onChange={e => changeMode(e.target.value as Mode)} disabled={busy} aria-describedby={busy ? 'mode-reason' : undefined}><option value="sandbox">My sandbox</option><option value="live">Shared demo records (read-only)</option></select>{busy && <small id="mode-reason">Mode switch paused while a change is saved.</small>}</div></header>
