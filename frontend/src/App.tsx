...
export function App() {
  ...
  const aboutDemo = useRef<HTMLDetailsElement>(null);
  const confirmFresh = useRef<HTMLButtonElement>(null);
  ...
  return <div className="app-shell"><a href="#main" className="skip-link" onClick={e => { e.preventDefault(); document.getElementById('main')?.focus(); }}>Skip to main content</a>
    ...
    <div className="workspace"><header className="topbar"><div className="date-group"><span className="date-chip">Today · <time dateTime={today}>{today}</time></span><small className="date-basis">Browser date at view opening · reload to update</small><div className="sr-only"><ClockBasis today={today}/></div></div><div className="mode-control"><label htmlFor="mode">Workspace</label><select id="mode" value={mode} onChange={e => changeMode(e.target.value as Mode)} disabled={busy} aria-describedby={busy ? 'mode-reason' : undefined}><option value="sandbox">My sandbox</option><option value="live">Shared demo records</option></select>{busy && <small id="mode-reason">Mode switch paused while a change is saved.</small>}</div></header>
      <div className="demo-banner"><span className="demo-dot" aria-hidden="true"/><strong>Synthetic demo</strong><span>{mode === 'sandbox' ? 'Sandbox · synthetic data · nothing is published' : data?.mode === 'live' && data.can_write ? 'Shared records · synthetic data · your approval publishes a public record' : 'Shared demo records · read-only · synthetic data'}</span></div>
      <main id="main" tabIndex={-1}>
        {error && !['landing', 'journeys', 'architecture', 'impact'].includes(route.page) && (actionBlocked ? <SessionRecovery key={`recovery-${sessionEpoch}`} message={error} sessionUnavailable={expired} mode={mode} busy={loading || busy} hasSnapshot={!!data} refresh={() => void refresh()} restart={() => void restart()}/> : route.page === 'intake' ? null : <div role="alert" className="error"><h2>That action could not be completed</h2><p>{error}</p><p>Correct the highlighted information and submit the offer again. The rejected intake was not saved.</p><button className="secondary" disabled={loading || busy} aria-describedby={loading || busy ? 'review-reason' : undefined} onClick={() => void refresh()}>Refresh and review</button>{(loading || busy) && <p id="review-reason" className="small-note">Refresh paused while the workspace loads or saves.</p>}</div>)}
        ...
        {route.page === 'landing' ? <LandingPage data={data} onLaunchCockpit={() => navigateTo('/dashboard')} onNavigate={navigateTo}/> : route.page === 'journeys' ? <UserJourneysView data={data}/> : route.page === 'architecture' ? <ArchitectureView/> : route.page === 'impact' ? <GtmImpactView/> : (data && <div key={sessionEpoch}>{route.path === '/previous-workspace' ? <div className="empty"><h1>This address belongs to the previous workspace</h1><p>The new session cannot restore the old offer or its approvals. No action was repeated.</p><a href="#/dashboard">Open the current dashboard</a></div> : route.page === 'dashboard' ? <Dashboard data={data} today={today} unit={route.unit} selected={selected} pickup={route.pickup} onStartOver={mode === 'sandbox' && !actionBlocked ? requestRestart : undefined}/> : route.page === 'workspace' ? <DispatchWorkspace key={`${mode}-${route.filter}-${route.unit}`} data={data} selected={selected} filter={route.filter} unit={route.unit} today={today} busy={unavailable} mutate={mutate} pickup={route.pickup} onPickupChange={selectPickup}/> : route.page === 'pickups' ? <Pickups data={data} busy={unavailable} mutate={mutate}/> : route.page === 'history' ? <History data={data} selected={selected} pickup={route.pickup}/> : route.page === 'intake' ? <AddOffer key={mode} data={data} busy={unavailable} mutate={mutate} error={actionBlocked ? '' : error}/> : route.path === '/offers' ? <Offers key={mode} data={data}/> : route.page === 'records' ? <Records data={data} route={route} selected={selected} today={today}/> : <div className="empty"><h1>Page not found</h1><p>This address is not part of the workspace. <a href={routeLink('/workspace', { offer: selected, pickup: route.pickup })}>Open workspace</a>.</p></div>}</div>)}
      </main><footer><span>MERISMOS</span><span className="footer-refresh"><button className="text-button" onClick={() => void refresh()} disabled={loading || busy} aria-describedby={loading || busy ? 'refresh-reason' : undefined}>Refresh workspace</button>{(loading || busy) && <small id="refresh-reason">Refresh paused while the workspace loads or saves.</small>}</span><details className="about-demo" ref={aboutDemo}><summary>About this demo</summary><p>Five synthetic organisations and invented donations. The public sandbox runs a Strands agent loop with scripted responses: no model call, no message, no publication. Time saved and food rescued are not measured. Date cues are calendar dates, not food-safety checks or spoilage countdowns. Nothing is dispatched: there is no departure event, and a collection counts only once someone confirms it.</p><p className="small-note">{data?.provider || 'Connecting to the coordinator service'}{observedAt && <span className="observed-at">Snapshot as of {observedAt}{actionBlocked ? ' · Refresh failed' : loading ? ' · Refreshing' : ''}</span>}</p><ul><li><a href="#/overview">Overview</a></li><li><a href="#/journeys">How a donation moves</a></li><li><a href="#/architecture">AWS architecture</a></li><li><a href="#/impact">Impact and limits</a></li><li><a href="/acceptance.html">Automated acceptance</a></li><li><a href="/UAT.testbook.html">Acceptance testbook</a></li></ul><p className="small-note">Automated CI results; human acceptance is not run.</p>{mode === 'sandbox' && !actionBlocked && (confirmRestart ? <div className="form-actions"><button ref={confirmFresh} disabled={loading || busy} aria-describedby={loading || busy ? 'restart-reason' : undefined} onClick={() => { setConfirmRestart(false); void restart(); }}>Yes, start fresh</button><button className="secondary" onClick={() => setConfirmRestart(false)}>No, keep this sandbox</button>{(loading || busy) && <p className="small-note" id="restart-reason">Starting a fresh sandbox is paused while the workspace loads or saves.</p>}</div> : <button className="secondary" onClick={() => setConfirmRestart(true)}>Start over in a new sandbox</button>)}</details></footer></div></div>;
}
