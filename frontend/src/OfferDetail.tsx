...
export function DecisionPanel({ row, data, busy, mutate }: { row: OfferRow; data: Workspace; busy: boolean; mutate: Mutate }) {
  ...
  return <>
    ...
    {plan && <section className="panel padded approval-card"><p className="eyebrow">ONE DECISION · EXACT BYTES</p><h2>{plan.recorded ? 'The recorded plan' : 'Approve this exact plan'}</h2><p>{data.mode === 'sandbox' ? 'Sandbox approval records your decision only inside this synthetic session. It does not publish to the public record bucket.' : 'This approval authorizes a permanent public allocation record at the address below. Collection must still be confirmed separately.'}</p>{total !== null && <><table className="split-table"><caption className="sr-only">Amounts you are approving</caption><tbody>{shares.map(a => <tr key={a.org}><td>{a.org}</td><td className="nowrap">{`${a.quantity} ${offer.unit}`}</td></tr>)}</tbody></table><p>{`Left without a recipient: ${Number((offer.quantity - total).toFixed(6))} ${offer.unit}`}</p></>}<dl><dt>Record address</dt><dd className="break-all">{plan.key}</dd></dl><DigestCustody key={`${data.mode}-${plan.digest}-${plan.recorded}`} plan={plan} mode={data.mode}/><details><summary>Read the exact record text</summary><pre>{plan.body}</pre></details>
