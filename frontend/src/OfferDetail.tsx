...
export function DecisionPanel({ row, data, busy, mutate }: { row: OfferRow; data: Workspace; busy: boolean; mutate: Mutate }) {
  ...
  return <>
    <section className="panel padded action-card"><p className="eyebrow">NEXT DECISION</p><h2>{row.status === 'running' ? 'Working out the split' : plan?.recorded ? 'Arrange the collections' : 'Review the allocation'}</h2>
      {row.status === 'running' && <><p role="status" id={`${uid}-run`}>{row.progress?.stage || 'Starting the runner'} · {row.progress?.specialists_answered ?? 'Unknown number of'} specialists answered</p><ol className="run-steps"><li>Reading the offer</li><li>Specialists check food safety, capacity, equity and premises where each applies to this category</li><li>If a draft is made, a deterministic gate checks it for personal data</li><li>A split is proposed for your approval, or the run stops with the reason</li></ol></>}
