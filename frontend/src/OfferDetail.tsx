...
export function DecisionPanel({ row, data, busy, mutate }: { row: OfferRow; data: Workspace; busy: boolean; mutate: Mutate }) {
  ...
  return <>
    <section className="panel padded action-card"><p className="eyebrow">NEXT DECISION</p><h2>{row.status === 'running' ? 'Working out the split' : plan?.recorded ? 'Arrange the collections' : 'Review the allocation'}</h2><p>{result.note || 'Merismos reads the offer and the network’s food safety, capacity, equity and premises registers, then proposes a split with a reason for every organisation. Nothing moves until you approve.'}</p>
      {row.status === 'running' && <><p role="status" id={`${uid}-run`}>{row.progress?.stage || 'Starting the runner'} · {row.progress?.specialists_answered ?? 'Unknown number of'} specialists answered</p><ol className="run-steps"><li>Reading the offer</li><li>Four specialists check food safety, capacity, equity and premises</li><li>A deterministic gate checks the draft for personal data</li><li>A split is proposed for your approval</li></ol></>}
      ...
      {!plan?.recorded && busy && data.can_write && row.status !== 'running' && <p className="small-note" id={`${uid}-busy`}>Paused until the current change is saved or the error above is refreshed.</p>}
