...
export function intakeErrorField(detail: string): string {
  ...
  if (/phone|contact|person|address|iban|card|note/.test(text)) return 'note';
  return '';
}
...
export function AddOffer({ data, busy, mutate, error = '' }: { data: Workspace; busy: boolean; mutate: Mutate; error?: string }) {
  ...
  const invalid = error ? intakeErrorField(error) : '';
