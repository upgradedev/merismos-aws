...
export function Dashboard({ data, today, unit, selected }: { data: Workspace; today: string; unit: string; selected: string }) {
  ...
  return <>
    ...
    <section className="metric-grid" aria-label="Workspace metrics">{cards.map(card => <a className="metric-card" key={card.filter} href={routeLink('/records', { offer: selected, filter: card.filter, unit: currentUnit })}><span>{card.label}</span><strong>{card.value}</strong><small>{card.note}</small><span className="metric-drill">Inspect records ↗</span></a>)}</section>
    <div className="dashboard-grid"><section className="panel padded"><div className="section-heading"><h2>Priority work</h2><a href={routeLink('/workspace', { offer: selected })}>Open workspace →</a></div>{priority.length ? <div className="priority-list">{priority.map(row => <a key={row.offer.id} href={routeLink('/workspace', { offer: row.offer.id })}><div className="section-heading"><strong>{row.offer.title}</strong><Status value={row.status}/></div><span>{row.offer.quantity} {row.offer.unit} · {row.offer.donor}</span><DateCue offer={row.offer} today={today} closed={!!row.plan?.recorded}/></a>)}</div> : <Empty title="No priority offers">There are no unrecorded offers or pending pickups in the current scope.</Empty>}</section>
    <section className="panel padded"><div className="section-heading"><h2>Recent activity</h2><a href={routeLink('/history', { offer: selected })}>History →</a></div><ActivityList data={data} limit={5}/><p className="small-note">Available timestamps are shown as reported. Undated states are not treated as recent events.</p></section></div>
  </>;
}
