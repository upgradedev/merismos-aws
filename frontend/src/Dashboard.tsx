...
export function Dashboard({ data, today, unit, selected, pickup, onStartOver }: { data: Workspace; today: string; unit: string; selected: string; pickup?: string; onStartOver?: () => void }) {
  ...
  const cards: { label: string; value: string; note: string; filter: Filter }[] = [
    ...
    { label: 'Due soon or overdue', value: String(values.urgent), note: 'Unrecorded offers dated tomorrow, today or earlier', filter: 'urgent' },
  ];
  ...
  return <>
    ...
    <section className="panel padded coordinator-start" aria-label="Your next donation"><p className="eyebrow">ONE OFFER → A CONSIDERED SPLIT → A COLLECTION HANDOFF</p>
      {start.kind === 'completed' ? <><h2>{`Every donation in ${data.mode === 'sandbox' ? 'this sandbox' : 'these shared records'} is recorded${start.stopped ? ' or stopped with a reason' : ''}`}</h2>
        {onStartOver ? <><p>Review what happened in <a href={routeLink('/history')}>History</a>. Starting over opens a new sandbox with the synthetic sample offers; this sandbox is not changed or deleted.</p><button onClick={onStartOver}>Start over with a fresh sample</button></> : <a className="button" href={routeLink('/history')}>Review the history →</a>}</>
    ...
    <div className="dashboard-grid"><section className="panel padded"><div className="section-heading"><h2>Priority work</h2><a href={routeLink('/workspace', { offer: selected, pickup })}>Open workspace →</a></div>{priority.length ? <div className="priority-list">{priority.map(row => <a key={row.offer.id} href={routeLink('/workspace', { offer: row.offer.id })}><div className="section-heading"><strong>{row.offer.title}</strong><Status value={row.status}/></div><span>{row.offer.quantity} {row.offer.unit} · {row.offer.donor}</span><DateCue offer={row.offer} today={today} closed={!!row.plan?.recorded}/></a>)}</div> : <Empty title="No priority offers">There are no unrecorded offers or pending pickups in the current scope. <a href="#/offers/new">Add an offer</a></Empty>}</section>
