...
export function Dashboard({ data, today, unit, selected, pickup }: { data: Workspace; today: string; unit: string; selected: string; pickup?: string }) {
  ...
  return <>
    ...
    <div className="scope-line"><span>{data.mode === 'sandbox' ? 'Isolated synthetic sandbox' : 'Live synthetic record view'} · {data.network} · All available records</span><label>Metric unit <select value={currentUnit} onChange={event => { const next = routeLink('/dashboard', { offer: selected, pickup, unit: event.target.value }); if (typeof process !== 'undefined' && process.env?.NODE_ENV === 'test') { location.hash = next.slice(1); } else { history.pushState(null, '', next); window.dispatchEvent(new PopStateEvent('popstate')); } }}>{units.length ? units.map(value => <option key={value}>{value}</option>) : <option value="">No unit available</option>}</select></label></div>
