...
const FEEDBACK_LABELS: Record<string, string> = { driver_ready: 'Collector ready', recipient_ready: 'Recipient ready', no_show: 'No-show after agreed time' };
export function PickupCard({ item, data, busy, mutate, today }: { item: Pickup; data: Workspace; busy: boolean; mutate: Mutate; today: string }) {
  ...
  const [report, setReport] = useState('driver_ready');
  ...
  return <article className="panel padded pickup-card"><div className="section-heading"><Status value={item.state}/><span>{item.quantity} {item.unit}</span></div><h2>{item.org}</h2><a href={`#/offers/${item.offer_id}`}>{item.title}</a>
    ...
    {item.feedback?.length ? <section><h3>Reported handoff</h3><ul>{item.feedback.map((entry, index) => <li key={index}>{FEEDBACK_LABELS[entry.code] || entry.code.replaceAll('_', ' ')} · {entry.role} · {new Date(entry.at * 1000).toLocaleString()}</li>)}</ul><p className="small-note">Reported observations, not delivery confirmation or messages sent.</p></section> : null}
    {item.state === 'confirmed' && <p className="success-note"><strong>{data.mode === 'sandbox' ? 'Simulation confirmed' : 'Collection confirmed'}</strong> {data.mode === 'sandbox' ? 'in this synthetic sandbox' : 'in the coordinator register'}. This share cannot be claimed again. {typeof item.confirmed_at === 'number' && Number.isFinite(item.confirmed_at) && item.confirmed_at > 0 ? <time dateTime={new Date(item.confirmed_at * 1000).toISOString()}>{new Date(item.confirmed_at * 1000).toLocaleString()}</time> : 'Confirmation time unavailable.'}</p>}
    ...
    {!closed && data.can_write && (item.state === 'unclaimed' ? <form onSubmit={e => { e.preventDefault(); void mutate(item.offer_id, 'pickup', { ...payload, action: 'claim', role }); }}><label>Collecting role<select value={role} onChange={e => setRole(e.target.value)}>{data.roles.map(r => <option key={r}>{r}</option>)}</select></label><p className="small-note">Choose an organisational role. Do not enter a name or phone number.</p><button disabled={busy}>Claim this share</button></form> : <>
      ...
      <form onSubmit={async e => { e.preventDefault(); if (await mutate(item.offer_id, 'pickup', {...payload, action: 'feedback', feedback: report, role, consent: observed})) setObserved(false); }}><label>Handoff observation<select aria-label="Handoff observation" value={report} onChange={e => {setReport(e.target.value); setObserved(false);}}><option value="driver_ready">Collector ready</option><option value="recipient_ready">Recipient ready</option><option value="no_show">No-show after agreed time</option></select></label><label>Reporting role<select value={role} onChange={e => {setRole(e.target.value); setObserved(false);}}>{data.roles.map(value => <option key={value}>{value}</option>)}</select></label><label className="check-label"><input type="checkbox" checked={observed} onChange={e => setObserved(e.target.checked)}/>I observed this handoff event{data.mode === 'sandbox' ? ' in the simulation' : ''}.</label><button className="secondary" disabled={busy || !observed}>Save handoff report</button><p className="small-note">No notification is sent. No-show requires a past agreed time; it does not mark collection or erase the agreement.</p></form>
