...
export function Offers({ data }: { data: Workspace }) {
  ...
  return <>
    ...
    <section className="panel"><div className="toolbar"><h2>Donation offers</h2><div className="filters"><label><span className="sr-only">Search offers</span><input type="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="Search offers or donors"/></label><label><span className="sr-only">Filter status</span><select value={filter} onChange={e => setFilter(e.target.value)}><option value="all">All statuses</option>{['not_started', 'awaiting_approval', 'blocked', 'recorded', 'published'].map(s => <option key={s} value={s}>{labels[s]}</option>)}</select></label></div></div>
      {rows.length ? <div className="table-scroll"><table><caption className="sr-only">Current donation offers and next steps</caption><thead><tr><th scope="col">Offer / donor</th><th scope="col">Available</th><th scope="col">Collection date</th><th scope="col">Status</th><th scope="col">Next step</th></tr></thead><tbody>{rows.map(({ offer, status }) => <tr key={offer.id}><td><a className="offer-link" href={`#/offers/${offer.id}`}>{offer.title}</a><small>{offer.id} · {offer.donor}</small></td><td className="nowrap">{offer.quantity} {offer.unit}</td><td className="nowrap">{offer.collection_date}</td><td><Status value={status}/></td><td><a href={`#/offers/${offer.id}`} aria-label={`Review ${offer.title}`}>Review →</a></td></tr>)}</tbody></table></div> : <Empty title="No offers match">Clear your search or choose another status.</Empty>}
