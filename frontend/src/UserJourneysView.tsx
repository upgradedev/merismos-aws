...
const JOURNEYS: JourneyStep[] = [
  {
    ...
    number: '01',
    title: 'Intake',
    summary: 'The coordinator types what the donor organisation reported. Merismos refuses personal data and instruction-like text at the door, names what it refused, and marks the field when it can match it.',
    actor: 'Coordinator typing an organisation’s offer',
    ...
    artifactProduced: 'The offer JSON: title, donor organisation, quantity + unit, category (ambient / chilled / frozen), collection date, use-by, hours unrefrigerated, allergens and note',
  ...
  },
  {
    ...
    number: '02',
    title: 'The checks',
    summary: 'Up to four specialists apply deterministic rules first. Eligible specialists then run as Strands agents through bounded tools; a bounded solver proposes the split.',
    actor: 'Up to four rule checks; eligible checks use Strands agents',
    ...
    constraints: [
      ...
      'A live capacity block can create a one-shot wake; the sandbox records the reason but creates no schedule',
    ],
    ...
    deepDive: 'Each applicable specialist runs its deterministic rules first. A specialist whose rules refuse does not call a model; each remaining specialist reads the organisations register, allocation policy, retention policy and manifests through bounded read-only tools. A BeforeToolCallEvent hook cancels calls outside the allowed corpus. The sandbox drives the real Strands loop with a fixed scripted tool sequence and closing answer, with no Bedrock call. Terraform configures Amazon Bedrock for live runs; the exact frozen runtime must be confirmed by release evidence.',
  },
  {
    id: 'approval',
    number: '03',
    ...
    artifactProduced: 'Sandbox: approval state in the versioned workspace snapshot. Live mode: a Markdown record at a stable public address',
    deepDive: 'The coordinator sees every line and its reason before approving. Approval names the record digest and address it applies to; if the plan has changed since, the approval is refused. In the public sandbox the decision updates the versioned session snapshot and nothing is published. In live mode the writer Lambda, whose role is the only one of the three fleet roles with s3:PutObject on the records bucket, publishes a Markdown record. Corrections use new addresses that name what they replaced. History and index views mark supersession; the original raw object remains unchanged.',
  },
  {
    id: 'collection',
    number: '04',
    title: 'Collection',
    summary: 'A claim and explicit collection confirmation are separate. An agreed time is optional and stored separately. Merismos sends nothing.',
    actor: 'Coordinator selecting an organisational collecting role',
    timeToComplete: 'two required acts; time optional',
    constraints: [
      'Claim and confirmation are separate; an agreed time is optional and can be replaced before confirmation',
    ...
    awsServices: ['Amazon DynamoDB versioned workspace snapshot', 'SQLite workspace snapshot in CI'],
    artifactProduced: 'Versioned claim state: organisational role, claim status, optional agreed time and explicit confirmation',
    deepDive: 'Collection is recorded, not inferred. The coordinator selects an organisational role to claim a share, may save or replace an agreed time, and explicitly confirms that collection happened. These fields update the versioned workspace snapshot; they are not pickup events in the append-only run ledger. The selected role is not proof of a collector’s personal identity.',
  },
];
